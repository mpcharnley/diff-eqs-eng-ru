...
order method}} applies more generally to any second order linear homogeneous equation and the goal is the same: use one solution of the differential equation to generate another one.  The idea is that if we somehow found $y_1$ as a solution of
...
\end{equation}
\begin{tasks}
\task Does the {\it superposition principle} apply to this DE? Give a one- or two-sentence explanation either way.
