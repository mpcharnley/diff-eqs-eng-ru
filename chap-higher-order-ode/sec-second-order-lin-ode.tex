...
\begin{theorem1}[thm:secondEU]{Existence and uniqueness}\index{existence and uniqueness}
...
Since $e^{2x}$ and $e^{-5x}$ solve the linear differential equation in the previous example, we know that superposition applies, so that $C_1e^{2x} + C_2e^{-5x}$ solves the differential equation for any $C_1$ and $C_2$. The last thing to figure out is if this is \emph{all} of the solutions to this problem, namely if we can pick $C_1$ and $C_2$ in order to meet any possible initial condition $y(a) = b_0$ and $y'(a) = b_1$. If this is possible, then we know that our method using the characteristic equation to find $e^{2x}$ and $e^{-5x}$ as solutions was enough to always solve this problem. To begin, we'll define a structure that we'll use later in \Chapterref{linalg:appendix} and may seem a bit irrelevant now, but it will all tie together.

\begin{definition}[Linear Independence]
Let $y_1$, $y_2$, ... $y_n$ be any functions. We say that $\{ y_1, \ldots,  y_n\}$ are \index{linearly independent ! functions}\emph{linearly independent} if the only way to have the equation \[ C_1y_1 + C_2y_2 + \cdots + C_ny_n = 0 \] be true (as functions) is by choosing $C_1 = C_2 = \cdots = C_n = 0$. 
\end{definition}

Note that by having this true \emph{as functions}, we need to have that this combination is always zero, for all values of the independent variable. 
...
\begin{equation*}
C_1e^{2x} + C_2e^{-5x} = 0 \quad \Rightarrow \quad C_1e^{2x} = -C_2e^{-5x} \quad \Rightarrow \quad e^{7x} = -\frac{C_2}{C_1} 
\end{equation*}
which cannot be true for all values of $x$ because $e^{7x}$ is not a constant but $-\frac{C_2}{C_1}$ is. Another way is by taking the derivative of the combination and setting $x$ to zero in both the original and the derivative.
\begin{equation*}
\begin{split}
C_1e^{2x} + C_2e^{-5x} &= 0 \\
2C_1e^{2x} - 5C_2e^{-5x} &= 0\\
...
\end{split}
\end{equation*}
and we can solve this last system to get that we must have $C_1 = C_2 = 0$. 
\end{example}
...
In order to bring everything together, the last statement we need to show is that if two solutions are linearly independent, then their Wronskian is zero, which will tell us that linear independence is enough to know that we can solve any initial value problem we want and have found all solutions to the problem in question. 

\begin{theorem1}{Linear Independence and the Wronskian}
Let $p, q$ be continuous functions. Let $y_1$ and $y_2$ be two solutions to the homogeneous equation \eqref{sol:eqlinhom}. Then $y_1$ and $y_2$ are \index{linearly independent ! functions}\emph{linearly independent} if and only if the Wronskian $W(x) = y_1'(x)y_2(x) - y_2'(x)y_1(x)$ is non-zero for all $x$ where the differential equation is defined. 
\end{theorem1}
...
On the other hand, assume the solutions are linearly independent, and there is some point $x_0$ where $W(x_0) = 0$. This means that 
\begin{equation*}
y_1'(x_0)y_2(x_0) = y_1(x_0)y_2'(x_0) \quad \text{ or } \quad \frac{y_1'(x_0)}{y_1(x_0)} = \frac{y_2'(x_0)}{y_2(x_0)} \text{ assuming nothing is zero.}
\end{equation*}
This means that the pair of equations in \eqref{eqn:sysLinIndepW} are multiples of each other at $x_0$, meaning there is really only one equation, and so there are non-zero values of $C_1$ and $C_2$ that solve it. Letting $K_1$ and $K_2$ be these particular values, define
\begin{equation*}
\phi(x) = K_1y_1(x) + K_2y_2(x).
\end{equation*} 
Since $y_1$ and $y_2$ are solutions to \eqref{sol:eqlinhom} and the choice of $K_1$ and $K_2$ to solve \eqref{eqn:sysLinIndepW}, we know that $\phi$ solves \eqref{sol:eqlinhom} with $\phi(x_0) = \phi'(x_0) = 0$. However, $0$ is also a solution to this initial value problem and \thmref{thm:secondEU} says that solutions to this problem are unique. Therefore 
\begin{equation*}
0 = K_1y_1(x) + K_2y_2(x),
\end{equation*} 
contradicting the linear independence of $y_1$ and $y_2$. Therefore, $W(x)$ must be non-zero everywhere.
\end{proof}

Therefore, what this says is that for two solutions of a linear, homogeneous differential equation (which is more specific than just having two random functions), two solutions being linearly independent is equivalent to \eqref{eqn:linindepW} holding for any\footnote{%
...
Now, back to our discussion of constant coefficient equations. If $b^2 - 4ac > 0$, then we have two distinct real roots $r_1$ and $r_2$, giving rise to solutions of the form $y_1(x) = e^{r_1x}$ and $y_2(x) = e^{r_2x}$. Using condition \ref{eqn:linindepW} with $x_0 = 0$, we compute
\begin{equation*}
