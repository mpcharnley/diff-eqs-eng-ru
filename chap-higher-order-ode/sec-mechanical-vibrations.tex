...
\subsection{Physical Applications and Vertical Mass-Spring Systems}

The topics and terms discussed above can all be used to answer interesting questions involving physical systems. For example, the \emph{\myindex{envelope curves}} can tell how long you have to wait until a capacitor has less than a certain amount of charge on it, or until a mass-spring oscillation gets within a small threshold. The reason why this helps here is because the equation we need to solve for the end time value is much simpler and only has one solution. If $x_T$ is our threshold distance, then trying to solve 
\begin{equation*}
x_T = C e^{-pt} \cos ( \omega_1 t - \delta )
\end{equation*}
for a value of $t$ will likely have many solutions, while the equation 
\begin{equation*}
x_T = C e^{-pt} 
\end{equation*}
will only have one. The \emph{\myindex{quasi-frequency}} can tell at what rate a damped mass-spring system will oscillate and can allow us to solve for any of the related coefficients in the problem if the other ones are known. The different types of damping phenomena allow general phenomena to be analyzed without explicitly solving the entire problem.

In some cases, like the bungee jump mentioned before, the mass-spring system is oriented vertically instead of horizontally. In this scenario, it feels like the force of gravity is an applied force, which would make the equation non-homogeneous (and thus impossible to solve at this point). However, it we set up our coordinates correctly, this does not happen.

\begin{mywrapfigsimp}{1.9in}{2.3in}
\noindent
\begin{tikzpicture}
\tikzstyle{spring}=[thick,decorate,decoration={zigzag,pre length=0.15cm,post length=0.15cm,segment length=6}]
\draw[thick] (0,0) -- (3,0);
\draw[spring] (1,0) -- (1,-1);
\draw[spring] (2,0) -- (2,-2.5);
\draw (1.7, -2.5) -- (2.3, -2.5) -- (2.3, -3.1) -- (1.7, -3.1) --cycle;
\node at (2, -2.8) {$m$};
\draw[-latex, line width=2pt] (2, -3.1) -- (2,-3.9);
\node[right] at (2.1, -3.6) {$mg$};
\draw (0.8, -1) -- (1.0, -1) -- (0.9, -1) -- (0.9, -2.5) -- (0.8, -2.5) -- (1, -2.5);
\node[left] at (0.8, -1) {$x=0$};
\node[left] at (0.8, -2.5) {$x=L$};
\node[left] at (0.8, -1.7) {$L$};
\draw (2.3, -2.5) -- (2.5, -2.5);
\node[right] at (2.5, -2.5){$y=0$};
%\node[right] at (1.2, -0.5) {$k$};
\node[right] at (2.2, -1.3) {$k$};
\end{tikzpicture}
\end{mywrapfigsimp}
Assume that we have a mass-spring system, but oriented vertically. From a physics class, the formula
\begin{equation*}
F = k \Delta x
\end{equation*}
may be familiar for the force required to extend a spring by $\Delta x$ in length. If a mass is hanging on this spring, then the force we are applying is the force of gravity. Thus, we know that
\begin{equation*}
mg = k L
\end{equation*} where $m$ is the mass and $L$ is the length it extends the spring. If we start with our normal mass-spring system, we will get an equation that looks like
\begin{equation*}
mx'' + \gamma x' + kx = mg = kL
\end{equation*}
where we are taking the orientation that $x > 0$ when you extend the spring, i.e., in the downward direction. However, if we subtract the $kL$ to the other side, we can rewrite this as
\begin{equation*}
mx'' + \gamma x' + k(x-L) = 0 \qquad \Rightarrow \qquad my'' + \gamma y' + ky = 0
\end{equation*}
where $y = x-L$. Physically, we are choosing our zero point to be where the mass will eventually equilibrate (the place where it would hang if you let it go slowly) instead of where the spring would sit with no mass. Since we've chosen the point where the mass wants to stop and all of our solutions for unforced damped oscillations eventually go to zero, this lines up with this being a proper homogeneous problem that we are able to solve.
...
\task For an unknown object, you measure a quasi-frequency of \unit[0.2]{Hz}. What is the mass of the
